...
// const processLottery = async (bucket: string, file: string): Promise<void> => {
//   const logger = instance;
//   logger.info(`NEW FILE!: ${file}`);
//   getS3File(bucket, file).then((newFile) => {
//     convertToJSON(newFile, "Raw Lottery Rank").then((jsonRows) => {
//       jsonRows.forEach((jsonRow) => {
//         logger.info("HERE!!!!");
//         logger.info(
//           `The lottery file for ${jsonRow["Primary Applicant First Name"]} ${
//             jsonRow["Primary Applicant Last Name"]
//           } ${lotteryLineIsValid(jsonRow) ? "is" : "is not"} valid`
//         );
//       });
//     });
//   });
